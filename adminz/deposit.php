...
$count_stmt = $conn->prepare("SELECT COUNT(*) as total FROM deposits WHERE status = ?");
...
$deposit = $conn->prepare("SELECT d.*, ip.name as plan_name , w.network, w.coin_symbol
                           ...
                           ORDER BY d.id DESC 
                           LIMIT ? OFFSET ?");
$deposit->bind_param("sii", $status, $limit, $offset);
$deposit->execute();
...
<body class="bg-light" style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;">
    ...
    <main class="container my-4">
        ...
        <!-- Render Pagination UI Elements if multiple pages exist -->
        <?php if ($total_pages > 1): ?>
            <nav aria-label="Page navigation" class="d-flex justify-content-center">
                <ul class="pagination shadow-sm">
                    <!-- Previous Button -->
                    <li class="page-item <?= $current_pagination_page <= 1 ? 'disabled' : '' ?>">
                        <a class="page-link" href="?page=<?= $status ?>&p=<?= $current_pagination_page - 1 ?>" aria-label="Previous">
                    ...
                    <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                        <li class="page-item <?= $current_pagination_page === $i ? 'active' : '' ?>">
                            <a class="page-link" href="?page=<?= $status ?>&p=<?= $i ?>"><?= $i ?></a>
                        </li>
                    <?php endfor; ?>

                    <!-- Next Button -->
                    <li class="page-item <?= $current_pagination_page >= $total_pages ? 'disabled' : '' ?>">
                        <a class="page-link" href="?page=<?= $status ?>&p=<?= $current_pagination_page + 1 ?>" aria-label="Next">
